...
export default function About() {
    ...
    return (
        <section
            id="about"
            ref={ref}
            className="min-h-[90vh] flex items-center bg-theme-surface px-6 relative overflow-hidden"
        >
            {/* Floating Shape */}
            <motion.div
                className="absolute right-10 top-1/4 w-32 h-32 border border-theme-accent/20 rounded-full"
                style={{ y }}
                animate={{ rotate: 360 }}
                transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
            />
            <motion.div
                className="absolute left-10 bottom-1/4 w-20 h-20 bg-theme-accent/5 rounded-lg"
            ...
            <div className="max-w-3xl mx-auto text-center space-y-10 relative z-10">
                <motion.div
                    ...
                    className="mx-auto w-12 h-12 rounded-full bg-theme-accent/10 flex items-center justify-center mb-4"
                >
                    <motion.div
                        animate={{ scale: [1, 1.2, 1] }}
                        transition={{ duration: 2, repeat: Infinity }}
                    >
                        <Heart className="text-theme-accent" size={24} />
                    </motion.div>
                </motion.div>
                ...
                <motion.div
                    ...
                    className="h-px w-24 bg-theme-accent/30 mx-auto origin-center"
                />
            </div>
        </section>
    );
